...
/*      
      ...
      {     
		printf("==spsr value: %08b\n",GET_SPI(FCR_SPSR));
	      	ret = GET_SPI(FCR_SPDR);
		printf("==treg ret value: %08b\n",ret);
		
     		return ret;
      }
      printf("==error in sent 1 byte\n");
*/
...
{
if(bit) 
	SET_SPI(0x5, 0x11);		//cs high
else 
	SET_SPI(0x5, 0x01);		//cs low
}
...
{	//There are 7 steps need to do.(marked by [1]-[7])
	...
	if (CSLowRSV==0)
	{
		set_cs(1);//[6] CS High (if the CMD has data block response CS should be kept low)
	}
...
{  
	  ...
	  unsigned char cachedbuffer[512*4];
	  int i;
	  indexbuf = &cachedbuffer[0*512];
          Read_Single_Block(10240&~511,indexbuf);
	  printf("\n===read data from sd card address 10240: \n");
	  for(i = 0;i < 512; i++)
		  printf(" %02x",indexbuf[i]);
	  
	  indexbuf = &cachedbuffer[1*512];
	  for(i = 0;i < 512;i++)
	  	cachedbuffer[1*512+i] = i%16;		  
          Write_Single_Block(10240&~511,indexbuf);
	  
	  printf("===write data from sd card address 10240:\n");
	  indexbuf = &cachedbuffer[2*512];
          Read_Single_Block(10240&~511,indexbuf);
	  printf("\n===read data from sd card address 10240: \n");
	  for(i = 0;i <512; i++)
		  printf(" %02x",indexbuf[i]);
...
{  
	  ...
	  unsigned char cachedbuffer[512];
	  int i,j;
//sw:	128x512B
	  printf("====dump sdcard mem...\n");
	  
	  for(i = 0;i < 32*2000;i++){
	  	indexbuf = &cachedbuffer[0];
          	Read_Single_Block((i* 512)&~511,indexbuf);
	  	printf("\n===read data from sd card address block %d: \n",i);
	  	for(j = 0;j < 512; j++)
			  printf(" %02x",indexbuf[j]);
		printf("\n");
	  }
}
